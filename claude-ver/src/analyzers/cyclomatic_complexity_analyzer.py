...
class ImportNode:
    def __init__(self, name: str, path: Path, dependencies: Set[str], complexity: float, maintainability: float, cyclomatic_complexity: int):
        self.name = name
        self.path = path
    ...
    def __hash__(self):
        return hash((self.name, self.path, self.dependencies, self.complexity, self.maintainability, self.cyclomatic_complexity))
    
    def __eq__(self, other):    
        return self.name == other.name and self.path == other.path and self.dependencies == other.dependencies
...
class CyclomaticComplexityAnalyzer:
    ...
    def _analyze_file(self, file_path: Path):
        ...
        if self.enable_type_checking:
            self.type_errors = self._check_types(file_path, self.import_graph[file_path])
        else:
            self.type_errors = []
    ...
    def _generate_metrics(self):
        ...
        self.metrics["eigenvector_centrality"] = nx.eigenvector_centrality_numpy(self.import_graph)
        self.metrics["average_cohesion"] = np.mean([module["cohesion"] for module in self.cohesion_metrics.values()])
        self.metrics["average_eigenvector_centrality"] = np.mean(list(self.metrics["eigenvector_centrality"].values()))
        self.metrics["average_complexity"] = np.mean([module["complexity"] for module in self.modules.values()])
        self.metrics["average_maintainability"] = np.mean([module["maintainability"] for module in self.modules.values()])

        # Calculate advanced metrics
        self.metrics["average_cyclomatic_complexity"] = np.mean([module["cyclomatic_complexity"] for module in self.modules.values()])
        self.metrics["average_loc"] = np.mean([module["loc"] for module in self.modules.values()])
        self.metrics["average_sloc"] = np.mean([module["sloc"] for module in self.modules.values()])
        self.metrics["average_lloc"] = np.mean([module["lloc"] for module in self.modules.values()])
