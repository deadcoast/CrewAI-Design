...
class ImportNode:
    ...
    def __str__(self):
        return f"Module: {self.name}, Path: {self.path}, Dependencies: {self.dependencies}"
...
class ImportAnalyzer:
    ...
    def _analyze_complexity(self, file_path: Path):
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            source = f.read()
        complexity = cc_visit(source)
        self.modules[self._get_module_name(file_path)].cyclomatic_complexity = sum(c.complexity for c in complexity)
        return self.modules
        ...
        for cycle in self.cyclic_deps:
            if len(cycle) > 1:  # Exclude self-references
                ...
                for i in range(1, len(cycle)):
                    dep = cycle[i]
                    console.print(f"[yellow]Suggestion: Consider refactoring {module} to remove dependency on {dep}")
    ...
    def _calculate_module_depth(self, module):
        if module not in self.modules:
            return 0
        return 1 + max(self._calculate_module_depth(dep) for dep in self.modules[module].dependencies)
        
    def _calculate_complexity(self, module):
        if module not in self.modules:
            return 0
        return sum(self._calculate_complexity(dep) for dep in self.modules[module].dependencies)
